  ...
  return (
    <div className={`rounded-lg border bg-card transition-all duration-[250ms] ease-out ${expanded ? "tool-card-shadow-hover" : "tool-card-shadow hover:tool-card-shadow-hover"}`}>
      ...
      >
        ...
        <ChevronDown className={`h-4 w-4 shrink-0 text-muted-foreground transition-transform duration-[250ms] ${expanded ? "rotate-180" : ""}`} />
      </button>

      {expanded && (
        <div className="animate-fade-in-up border-t px-4 pb-5 pt-4 md:px-5">
            ...
            {result && (
              <div className="animate-fade-in-up flex items-center justify-between rounded-lg bg-accent/10 px-4 py-3">
                ...
                <div className="flex gap-1">
                  <button onClick={handleCopy} className="rounded-md p-1.5 hover:bg-accent/20 text-muted-foreground transition-colors" aria-label="Copy"><Copy className="h-4 w-4" /></button>
                  <button onClick={onReset} className="rounded-md p-1.5 hover:bg-accent/20 text-muted-foreground transition-colors" aria-label="Reset"><RotateCcw className="h-4 w-4" /></button>
                </div>
              </div>
            )}
          </div>
